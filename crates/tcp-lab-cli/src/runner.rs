...
use tcp_lab_core::{Simulator, SimConfig, TransportProtocol, TestScenario, TestAction, TestAssertion};
use tracing::info;
use std::fs;
...
pub fn run_scenario(
    ...
    // Max duration check
    let max_duration = scenario.assertions.iter().find_map(|a| {
        if let TestAssertion::MaxDuration { ms } = a { Some(*ms) } else { None }
    ...
    for assertion in &scenario.assertions {
        match assertion {
            TestAssertion::DataDelivered { data } => {
                ...
                if !found {
                    return Err(anyhow!("Assertion Failed: Data {:?} was not delivered", data));
                }
            },
            TestAssertion::SenderPacketCount { min, max } => {
                if sim.sender_packet_count < *min {
                    return Err(anyhow!("Assertion Failed: Sender sent {} packets, expected min {}", sim.sender_packet_count, min));
                }
                if let Some(max) = max {
                    if sim.sender_packet_count > *max {
                        return Err(anyhow!("Assertion Failed: Sender sent {} packets, expected max {}", sim.sender_packet_count, max));
                    }
                }
            },
            TestAssertion::SenderWindowMax { min, max } => {
                ...
                if max_win < *min {
                    return Err(anyhow!("Assertion Failed: Sender window max {} < expected min {}", max_win, min));
                }
                if let Some(m) = max {
                    if max_win > *m {
                        return Err(anyhow!("Assertion Failed: Sender window max {} > expected max {}", max_win, m));
                    }
                }
            },
            TestAssertion::SenderWindowDrop { from_at_least, to_at_most } => {
